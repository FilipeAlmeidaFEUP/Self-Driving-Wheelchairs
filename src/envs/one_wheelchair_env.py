...
class OneWheelchairEnv(Env):
    ...
    def step(self, action):
        ...
        if self.collisions:
            ...
            self.data['end_condition'].append('collision')
        elif self.finished:
            reward = 400 + ((self.max_episodes - ((self.episode) * 200) / self.max_episodes))
            done = True
            self.data['end_condition'].append('finished')
...
class OneWheelchairEnvWithDistance(Env):
    ...
    def __init__(self):
        ...
        self.observation_space = Box(0, 3, shape=(1,self.split + 7))
        while len(self.lidar_sample) == 0:pass
        self.state = np.array([0,0] + self.lidar_sample)
    ...
    def step(self, action):
        ...
        self.state = np.array([min(dist, 3),((angle/math.pi) * 1.5) + 1.5] + self.lidar_sample)
        ...
        #print('')
        #print(action,reward, (self.previous_diff[0]-dist) * (1000/6), (self.previous_diff[1]-angle) * (100/2))
    ...
    def sample_lidar(self,data):
        ...
        for i in range (len(self.lidar_sample)): self.lidar_sample[i] = min(self.lidar_sample[i], 3)
